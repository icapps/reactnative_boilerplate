    ...
    class UpdateTranslationsAction < Action
      def self.run(params)
        ...
        if changed_files.count == 0
          UI.message "Didn't commit any changes, no translations were updated.".yellow
        else
          begin
            Actions.sh("git add #{changed_files.map(&:shellescape).join(' ')}")
            Actions.sh("git commit -m '#{params[:commit_message]}'")

            UI.message "Committed \"#{params[:commit_message]}\" 💾.".green
          rescue => ex
            UI.error ex
            UI.message "Didn't commit any changes.".yellow
          end
        end
      end
